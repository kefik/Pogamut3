...
import eu.profinit.maven.plugin.dependency.utils.filters.ArtifactItemFilter;
import eu.profinit.maven.plugin.dependency.utils.filters.MarkerFileFilter;
import eu.profinit.maven.plugin.dependency.utils.markers.MarkerHandler;
    ...
    extends AbstractFromConfigurationMojo
{
	...
	/**
	 * If set to true, will not unpack an artifact if the marker-file for the exact version of the artifact already
	 * exists inside (usually) "target/dependency-maven-plugin-markers".
	 * 
	 * @parameter expression="${mdep.unpack.donotunpackifmarkerexists}" default-value="false"
	 */
	private boolean doNotUnpackIfMarkerExists;
        ...
        throws MojoExecutionException
    {
        ...
        if (doNotUnpackIfMarkerExists) {
	        if (handler.isMarkerSet()) {
	        	if (handler.isMarkerOlder(artifactItem.getArtifact())) {
	        		getLog().info("doNotUnpackIfMarkerExists set to true & marker for the artifact (" + artifactItem.toString() + ") exists but it marks OLDER version of the artifact, unpacking new one");
	        	} else {
	        		getLog().info("doNotUnpackIfMarkerExists set to true & marker for the same (or newer) artifact (" + artifactItem.toString() + ") exists, SKIPPING UNPACK for this artifact.");
	        		getLog().info("    - If you wish the artifact to be re-unpacked, delete '" + handler.getMarker() + "'.");
	        		return;
	        	}
	        }
        }
