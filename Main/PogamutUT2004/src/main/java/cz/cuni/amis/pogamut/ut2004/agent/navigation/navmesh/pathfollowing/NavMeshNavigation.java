...
public class NavMeshNavigation implements IUT2004Navigation {
    ...
    private void navigateLocation() {
        ...
        if (currentPathFuture == null) {
        	...
        	ILocated fromLocation = (isOnNavMesh() ? bot.getLocation() : getNearestNavPoint(bot.getLocation()));
        	fromNavPoint = (fromLocation instanceof NavPoint ? (NavPoint)fromLocation : null);
        	toNavPoint = getNearestNavPoint(currentTarget);

            if (log != null && log.isLoggable(Level.FINE)) {
                log.fine("  +-- Computing path from " + (fromNavPoint == null ? fromLocation : fromNavPoint.getId().getStringId()) + " to " + toNavPoint.getId().getStringId());
            }

            currentPathFuture = pathPlanner.computePath(fromLocation, toNavPoint);
        }
    ...
    private void checkExtendPath() {
        ...
        if (isPathExecuting()) {
            ...
            if (remainingDistance < extendPathThreshold) {
                ...
                log.fine(" +-- Extending path to continue to " + continueTo);
    ...
    private void navigatePlayer() {
        ...
        if (hDistance < AT_PLAYER && vDistance < 50) {
            ...
            if (pathExecutor.isExecuting()) {
            	// WE NEED PATH EXECUTOR TO CORRECTLY FINISH
        ...
        if (hDistance < 300 && Math.abs(vDistance) < 50) {
            // RUN STRAIGHT			
        ...
        if (pathExecutor.isExecuting()) {
            ...
            ILocated newToLocation = (isOnNavMesh(currentTargetPlayer) ? currentTargetPlayer : getNearestNavPoint(currentTargetPlayer));
            NavPoint newToNavPoint = (newToLocation instanceof NavPoint ? (NavPoint)newToLocation : getNearestNavPoint(newToLocation));
            if (isOnNavMesh() || newToNavPoint != toNavPoint) {
