...
public class OldNavMeshConstants {
        ...
        if(x==0) {
            ...
            if(y==0) {
                ...
                if(y>0) {
                    // 0-90
                    if(x>0) {
                        yaw = 0*UTQuarterAngle + Math.atan(y/x)/(2*Math.PI)*UnrealUtils.FULL_ANGLE_IN_UNREAL_DEGREES;
                    }
                    // 90-180
                    else {
                        yaw = 1*UTQuarterAngle + Math.atan(-x/y)/(2*Math.PI)*UnrealUtils.FULL_ANGLE_IN_UNREAL_DEGREES;
                    }
                }
                // 180-360
                else {
                    // 180-270
                    if(x<0) {
                        yaw = 2*UTQuarterAngle + Math.atan(-y/-x)/(2*Math.PI)*UnrealUtils.FULL_ANGLE_IN_UNREAL_DEGREES;
                    }
                    // 270-360
                    else {
                        yaw = 3*UTQuarterAngle + Math.atan(x/-y)/(2*Math.PI)*UnrealUtils.FULL_ANGLE_IN_UNREAL_DEGREES;
                    }    
                }
            }
        }        
    ...
    public static Vector2D transformRotationTo2DVector(double yaw) {
            ...
            while(yaw < 0) yaw += UnrealUtils.FULL_ANGLE_IN_UNREAL_DEGREES;
            yaw = yaw % UnrealUtils.FULL_ANGLE_IN_UNREAL_DEGREES;
            // transform it to radians
            double yawRad = yaw / UnrealUtils.FULL_ANGLE_IN_UNREAL_DEGREES * 2*Math.PI;
