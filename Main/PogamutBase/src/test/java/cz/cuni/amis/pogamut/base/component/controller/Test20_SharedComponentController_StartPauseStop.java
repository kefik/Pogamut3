...
/**
 * Tests: start/pause/stop
 * @author Jimmy
 *
 */
import cz.cuni.amis.tests.BaseTest;
				
public class Test20_SharedComponentController_StartPauseStop extends BaseTest {
	
	@Test
	public void test() {
		IAgentId agentId1 = new AgentId("Test20_Shared-Agent1");
		IAgentId agentId2 = new AgentId("Test20_Shared-Agent2");
		...
		if (sharedComp.getController().notInState(ComponentState.PAUSED)) {
			String msg = "[ERROR] " + sharedComp.getComponentId().getToken() + " is not in state PAUSED but in state " + sharedComp.getController().getState().getFlag() + ", INVALID!";
			System.out.println(msg);
			throw new RuntimeException(msg);
		}
