...
public class Test05_AgentRunner_PausingBehavior extends BaseTest {
	...
	protected IAgentRunner initAgentRunner() {
		...
		return new TestAgentRunner(new GuiceAgentFactory(new TestAgent1Module())) {
			...
			protected void preResumeHook(List<AbstractAgent> agents) {
				...
				if (preInitHookCalled == 0) throw new RuntimeException("preResumeHook() called but no preInitHook was called!");
				if (preInitHookCalled != 1) throw new RuntimeException("preResumeHook() called when more than one preInitHook was called!");
				if (preResumeHookCalled != 0) throw new RuntimeException("preResumeHook() called repeatedly!");
				if (preStartHookCalled != agents.size()) throw new RuntimeException(preStartHookCalled + " agent started, but param agents.size() == " + agents.size() + "!");
				for (int i = 0; i < agents.size(); ++i) {
					if (!agents.get(i).getState().getFlag().isState(IAgentStatePaused.class)) throw new RuntimeException("preResumeHook called, but param agents[" + i + "] is not in the state IAgentStatePaused");
				}
				++preResumeHookCalled;
			};
		}.setPausing(true);
	...
	protected void checkAgentRunnerCalls(int numAgents) {
		...
		if (preResumeHookCalled == 0) throw new RuntimeException("preResumeHook() was not called at all!");
		if (preResumeHookCalled != 1) throw new RuntimeException("preResumeHook() was called more than once!");
	...
	public static void main(String[] args) {
		Test05_AgentRunner_PausingBehavior test = new Test05_AgentRunner_PausingBehavior();
		test.test_start1Agent();
		test.test_start5Agents();
		test.test_start5AgentsWithDifferentParams();
		Test05_AgentRunner_PausingBehavior.tearDown();
	}
