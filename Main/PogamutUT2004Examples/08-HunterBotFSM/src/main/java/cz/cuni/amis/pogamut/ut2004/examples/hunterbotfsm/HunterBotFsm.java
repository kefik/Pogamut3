 ...
 */
@AgentScoped
public class HunterBotFsm extends UT2004BotModuleController<UT2004Bot> {
	...
	private FSM<HunterBotFsm> fsm = new FSM<HunterBotFsm>(new PickItemsState());
    ...
    public void prepareBot(UT2004Bot bot) {
        ...
        weaponPrefs.newPrefsRange(1000).add(UT2004ItemType.LINK_GUN, false)
		  							   .add(UT2004ItemType.MINIGUN, true);
        							  
        weaponPrefs.newPrefsRange(5000).add(UT2004ItemType.LIGHTNING_GUN, false)
    ...
    protected void reset() {
        ...
        fsm = new FSM<HunterBotFsm>(new PickItemsState());
    }
    ...
    private class PickItemsState extends State<HunterBotFsm> {

        @Override
        public State<HunterBotFsm> execute(HunterBotFsm context) {
            if (context.shouldEngage && context.players.canSeeEnemies() && context.weaponry.hasLoadedWeapon())
                return new EngageState();
            
            if (context.senses.isBeingDamaged())
                return new BeingHitState();
            
            if (context.enemy != null && context.shouldPursue && context.weaponry.hasLoadedWeapon())
                return new PursueState();
            
            if (context.shouldCollectHealth && context.info.getHealth() < context.healthLevel)
                return new HealState();
            
            context.stateRunAroundItems();
            return this;
        }

    }
    
    private class EngageState extends State<HunterBotFsm> {
        ...
        public State<HunterBotFsm> execute(HunterBotFsm context) {
            ...
            if (info.isShooting() || info.isSecondaryShooting())
                context.getAct().act(new StopShooting());
            ...
            return new PickItemsState();
        }
    }
    
    private class BeingHitState extends State<HunterBotFsm> {
    ...
    private class PursueState extends State<HunterBotFsm> {
    ...
    private class HealState extends State<HunterBotFsm> {
    ...
    public static void main(String args[]) throws PogamutException {
        ...
        new UT2004BotRunner(HunterBotFsm.class, "Hunter").setMain(true).setLogLevel(Level.INFO).startAgents(2);
    }
}
